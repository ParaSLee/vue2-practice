...
var jserver = jsonServer.create(); //创建服务器
var jrouter = jsonServer.router('db.json') //引入json 文件 ，这里的地址就是你json文件的地址
var middlewares = jsonServer.defaults(); //返回JSON服务器使用的中间件。
jserver.use(middlewares)
jserver.use(jrouter)
jserver.listen( 3000,function(){ //json服务器端口:比如你使用8080,这里的json服务器就是8081端口
